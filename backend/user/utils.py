...
def send_reset_link_email(email, reset_link):
    ...
    return send_mail(subject, message, from_email, [email]) > 0

def send_email_to_client(email, link):
    try:
        subject = "Your WoofWorld Registration OTP"
        html_message = get_otp_email_template(link)
        ...
        send_mail(
            subject=subject,
            message=f"Your OTP is: {link}",  # Plain text fallback
